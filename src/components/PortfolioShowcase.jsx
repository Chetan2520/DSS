...
const projects = [
  ...
  },
  {
    ...
    accent:
      "group-hover:shadow-orange-500/30 border-orange-500/50 text-orange-400",
  ...
  },
  {
    ...
    accent:
      "group-hover:shadow-green-500/30 border-green-500/50 text-green-400",
...
export default function PortfolioShowcase() {
  ...
  const handleMouseEnter = (e) => {
    ...
    if (img) {
      ...
      const scrollHeight =
        img.offsetHeight - card.querySelector(".viewport").offsetHeight;
      if (scrollHeight > 0) {
        gsap.to(img, {
          y: -scrollHeight,
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        {/* Blue Glow (Top Left) */}
        <div
          className="absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] bg-[#0078f0]/20 rounded-full blur-[120px] animate-pulse"
          style={{ animationDuration: "8s" }}
        />

        {/* Orange Glow (Bottom Right) */}
        <div
          className="absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] bg-[#ff9f20]/15 rounded-full blur-[120px] animate-pulse"
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div ref={titleRef} className="text-center mb-20 will-change-transform">
          <div className="flex items-center justify-center gap-4 text-gray-500 text-xs font-mono   tracking-[0.3em] mb-6">
            <div className="w-12 h-[1px] bg-gray-700" />
            <span>Our Portfolio</span>
            <div className="w-12 h-[1px] bg-gray-700" />
          </div>

          <h2 className="text-5xl md:text-7xl   text-white tracking-tighter mix-blend-exclusion">
            Something
            <br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
          {projects.map((project, i) => (
            ...
            >
              {/* --- BROWSER FRAME CARD --- */}
              <div
                className={`relative w-full aspect-[4/3] bg-[#111] rounded-xl overflow-hidden border border-white/10 transition-all duration-500 group-hover:border-opacity-50 group-hover:shadow-2xl hover:-translate-y-2 ${project.accent}`}
              >
                {/* Internal Glow Gradient */}
                <div
                  className={`absolute inset-0 bg-gradient-to-b ${project.theme} opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`}
                />
                ...
                <div className="viewport absolute inset-0 top-8 bg-gray-900 overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/40 group-hover:opacity-0 transition-opacity duration-500 pointer-events-none flex items-center justify-center">
                    <div className="px-5 py-2 bg-white/5 backdrop-blur-md border border-white/10 rounded-full text-[10px] font-bold   tracking-widest text-white/80 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                      View Case Study
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-start px-2">
                <div>
                  <h3
                    className={`text-2xl md:text-3xl font-bold text-white mb-2 transition-colors duration-300 group-hover:${project.accent.split(" ").find((c) => c.startsWith("text-"))}`}
                  >
                    {project.title}
                  </h3>
