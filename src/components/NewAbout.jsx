...
export default function BrandParallax() {
  ...
  const dotsData = useRef(
    [...Array(60)].map(() => {
      // Reduced from 100 to 60 for mobile performance (looks same)
  ...
  const infoPoints = [
    ...
    },
    {
      id: 2,
      title: "Design",
      desc: "Crafting visual identities.",
    ...
    <div className="bg-[#050505] text-white">
      ...
      >
        ...
        <div className="absolute inset-0 z-0">
          ...
          >
            <source
              src="https://www.pexels.com/download/video/4919748/"
              type="video/mp4"
            />
          </video>
        </div>
        ...
        <div
          ...
          style={{
            ...
            maskImage:
              "radial-gradient(circle at center, transparent var(--mask-radius), black calc(var(--mask-radius) + 60px))",
            WebkitMaskImage:
              "radial-gradient(circle at center, transparent var(--mask-radius), black calc(var(--mask-radius) + 60px))",
          }}
        >
          ...
          {infoPoints.map((item) => (
            ...
            >
              <h3 className="text-3xl md:text-5xl font-bold text-white mb-2 tracking-tighter mix-blend-difference will-change-transform">
                {item.title}
                <span className="text-[#0078f0] inline-block scale-150 leading-none">
                  .
                </span>
              </h3>

              <div
                className={`flex items-center gap-3 justify-center ${item.align.includes("right") ? "md:flex-row-reverse" : "md:flex-row"}`}
              >
                <div className="hidden md:block h-[1px] w-12 bg-[#0078f0]/50" />
                <p className="text-xs md:text-sm font-light text-gray-300   tracking-[0.2em]">
                  {item.desc}
                </p>
              </div>
            </div>
