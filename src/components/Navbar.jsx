...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        ref={navRef}
        className={`${isSubPage ? "sticky bg-black py-4 border-b border-white/10" : "fixed bg-transparent py-5"} top-0 left-0 w-full z-50 transition-all duration-300 ${
      ...
      >
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-10">
            <button onClick={() => handleNavClick("/")} className="text-sm md:text-lg text-white hover:text-white transition-colors cursor-pointer tracking-wide">Home</button>
            <button onClick={() => handleNavClick("/about")} className="text-sm md:text-lg text-white hover:text-white transition-colors cursor-pointer tracking-wide">Who We Are</button>
              ...
              >
                ...
                <div className="w-[35%] p-8 space-y-1">
                  ...
                  {services.map((service) => (
                    ...
                    >
                      <div className={`shrink-0 ${activeService.name === service.name ? "text-orange-500 scale-110" : "text-white/30 group-hover/item:text-white group-hover/item:scale-110"} transition-all`}>
                        <service.icon size={18} />
                      </div>
                      <span className={`text-sm font-bold transition-all ${activeService.name === service.name ? "text-white" : "text-white/60 group-hover/item:text-white"}`}>
                        {service.name}
                      </span>
                    </div>
                  ))}
                ...
                <div className="w-[65%] p-10  flex flex-col items-center">
                  <div key={activeService.name} className="animate-in fade-in slide-in-from-right-8 duration-700 flex flex-col h-full w-full">
                    {/* Image at Top */}
            ...
            <button onClick={() => handleNavClick("/portfoliopage")} className="text-sm md:text-lg text-white hover:text-white transition-colors cursor-pointer tracking-wide">Portfolio</button>
            <button onClick={() => handleNavClick("/blogs")} className="text-sm md:text-lg text-white hover:text-white transition-colors cursor-pointer tracking-wide">Blogs</button>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <span className={`w-5 h-[1.5px] bg-white transition-all duration-300 ${isMobileMenuOpen ? "rotate-45 translate-y-1.5" : ""}`} />
              <span className={`w-5 h-[1.5px] bg-white transition-all duration-300 ${isMobileMenuOpen ? "opacity-0 scale-0" : ""}`} />
              <span className={`w-5 h-[1.5px] bg-white transition-all duration-300 ${isMobileMenuOpen ? "-rotate-45 -translate-y-1.5" : ""}`} />
            </button>
          </div>
        </div>
      ...
      {/* Mobile Menu */}
      <div ref={overlayRef} className="fixed inset-0 bg-black/60 backdrop-blur-md z-40 lg:hidden opacity-0 invisible" onClick={() => setIsMobileMenuOpen(false)} />
      <div ref={mobileMenuRef} className="fixed top-24 left-6 right-6 z-40 lg:hidden bg-[#0d0d0d] border border-white/5 rounded-[2rem] p-8 opacity-0 -translate-y-4 shadow-2xl overflow-y-auto max-h-[80vh]">
        <nav className="flex flex-col gap-2">
              ...
              {link.submenu ? (
                <>
                  <button 
                    ...
                    className="flex items-center justify-between text-lg font-bold text-white/70 hover:text-white w-full text-left py-4 border-b border-white/5 transition-colors cursor-pointer tracking-widest uppercase text-xs"
                  >
                    {link.name}
                    <ChevronDown size={16} className={`transition-transform duration-300 ${activeMobileSubmenu === link.name ? "rotate-180" : ""}`} />
                  </button>
                  <div className={`overflow-hidden transition-all duration-300 ${activeMobileSubmenu === link.name ? "max-h-[500px] opacity-100 py-4" : "max-h-0 opacity-0"}`}>
          ...
          <button onClick={() => handleNavClick("/LetsConnect")} className="mt-6 w-full py-4 bg-[#0078f0] text-white font-bold text-center rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-transform cursor-pointer tracking-widest text-[11px] uppercase">
            Start Project
          </button>
        </nav>
      </div>
