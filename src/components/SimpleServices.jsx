        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((item, index) => (
            ...
            >
              ...
              <div
                className="absolute inset-0 rounded-3xl bg-transparent opacity-50 group-hover:opacity-100 transition-opacity duration-500"
                style={{
              ...
              <div className="relative h-full bg-[#0a0a0a]/90 backdrop-blur-xl rounded-3xl p-6 flex flex-col justify-between overflow-hidden border border-white/5 group-hover:border-white/10 transition-colors">
                {/* Hover Inner Glow */}
                <div
                  className={`absolute -top-20 -right-20 w-64 h-64 bg-gradient-to-br ${item.gradient} blur-[60px] opacity-0 group-hover:opacity-100 transition-all duration-500`}
                />
                ...
                <div>
                  ...
                  {/* Title & Description */}
                  <h3 className="text-2xl   text-white mb-3 group-hover:text-blue-400 transition-colors   tracking-tight">
                    {item.title}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    {item.features.map((feature, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 text-[10px]   text-gray-300 bg-white/5 border border-white/5 rounded-full flex items-center gap-1.5   tracking-widest"
                      >
                        <span
                ...
                <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-auto">
                  <span className="text-xs   text-white group-hover:tracking-widest transition-all duration-300  ">
                    Explore Service
                  </span>
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-white/5 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                    <ArrowUpRight
                      size={16}
