...
export default function BrandProcess() {
  ...
  return (
    <div className="bg-[#050505] relative text-white font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      <div
        ref={containerRef}
        className="relative h-screen flex flex-col justify-center bg-[#050505] overflow-hidden"
      >
        {/* Background Grid */}
        ...
        <div className="flex-none h-[25vh] md:h-[30vh] flex flex-col justify-center items-center text-center px-6 relative z-10 border-b border-white/5">
          <h2 className="text-4xl md:text-8xl   text-white tracking-tighter leading-none mb-2 will-change-transform  ">
            Zero to Brand
          </h2>
          <div className="flex items-center gap-3 text-gray-500 text-xs font-mono   tracking-[0.2em]">
            <span>Scroll</span>
          ...
          >
            {steps.map((step, index) => (
              ...
              >
                {/* 1. IMAGE LAYER */}
                <div
                  className={`w-full h-full ${step.theme} transition-all duration-700 relative overflow-hidden`}
                >
                  <img
                    src={step.img}
                    alt={step.title}
                ...
                <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 z-20">
                  <div
                    className={`text-5xl md:text-7xl   opacity-10 absolute -top-16 left-6 select-none transition-opacity group-hover:opacity-20 ${step.accent.split(" ")[0]}`}
                  >
                    {step.id}
                  </div>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div
                      className={`w-2 h-2 rounded-full ${step.accent.includes("blue") ? "bg-blue-500" : step.accent.includes("purple") ? "bg-purple-500" : step.accent.includes("orange") ? "bg-orange-500" : step.accent.includes("yellow") ? "bg-yellow-400" : "bg-gray-500"}`}
                    />
                    <span
                      className={`text-xs font-bold   tracking-[0.2em] ${step.accent.split(" ")[0]}`}
                    >
                      {step.phase}
                    </span>
                  </div>
            ...
            <div className="relative shrink-0 w-[80vw] md:w-[600px] h-[55vh] md:h-[60vh] flex items-center justify-center border border-white/5 bg-white/[0.02] backdrop-blur-sm">
              <div className="text-center p-8">
                <p className="text-gray-500   tracking-widest text-xs mb-4">
                  Ready to start?
                </p>
                <h3 className="text-4xl md:text-6xl   text-white leading-none mb-8">
                  LET'S BUILD
                  <br />
                ...
                <button className="group relative px-8 py-3 bg-white text-black font-bold   tracking-widest overflow-hidden hover:bg-transparent hover:text-white border border-transparent hover:border-white transition-all duration-300">
