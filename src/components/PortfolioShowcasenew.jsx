...
const ProjectCard = React.memo(({ project }) => {
  ...
  const handleMouseEnter = () => {
    ...
    const moveY =
      imgRef.current.offsetHeight - viewportRef.current.offsetHeight;
  ...
  return (
    ...
    >
      <div
        className={`relative w-full aspect-[4/3] bg-[#111] rounded-xl overflow-hidden border border-white/10 transition-all duration-300 group-hover:border-opacity-50 group-hover:shadow-2xl hover:-translate-y-1 ${project.accent}`}
      >
        {/* Placeholder - Absolute minimum DOM */}
        {!isLoaded && <div className="absolute inset-0 bg-zinc-900 z-30" />}
        ...
        <div
          ref={viewportRef}
          className="viewport absolute inset-0 top-8 bg-gray-900 overflow-hidden relative"
        >
          <img
            ref={imgRef}
            src={project.img}
          ...
          <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none transition-colors duration-300 group-hover:bg-black/40">
            {isLoaded && (
              <div className="px-5 py-2 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-[10px] font-bold   tracking-widest text-white opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                View Project
              </div>
            )}
          </div>
      ...
      <div className="flex justify-between items-start px-2">
        <div>
          <h3
            className={`text-xl md:text-2xl font-bold text-white mb-2 transition-colors duration-300 group-hover:${project.accent.split(" ").find((c) => c.startsWith("text-"))}`}
          >
            {project.title}
          </h3>
...
export default function PortfolioPage() {
  ...
  return (
    <div
      ref={containerRef}
      className="bg-[#050505] text-white min-h-screen pt-32 pb-20 font-sans selection:bg-blue-500/30"
    >
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="text-center mb-16">
          <h1 className="pf-header text-5xl md:text-8xl   mb-6 tracking-tighter">
            Selected{" "}
          ...
          <p className="pf-header text-zinc-400 text-lg max-w-2xl mx-auto">
            A showcase of digital excellence. From pixel-perfect websites to
